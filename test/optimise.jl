...
@testset "IntervalOptimisation tests" begin
    @testset "numeric_type" begin
        x = -10..10
        big_x = big(x)
        @test IntervalOptimisation.numeric_type(x) == Float64
        @test IntervalOptimisation.numeric_type(big_x) == BigFloat
        @test IntervalOptimisation.numeric_type([x, x]) == Float64
        @test IntervalOptimisation.numeric_type([big_x, big_x]) == BigFloat
    end

    @testset "Minimise in 1D using default data structure i.e HeapedVector" begin
        global_min, minimisers = minimise(x->x, -10..10)
        @test issubset_interval(global_min, -10 .. -9.999)
        @test length(minimisers) == 1
        @test issubset_interval(minimisers[1], -10 .. -9.999)

        # same but with maximise
        global_max, maximisers = maximise(x->x, -10..10)
        @test issubset_interval(global_max, 9.999 .. 10)
        @test length(maximisers) == 1
        @test issubset_interval(maximisers[1], 9.999 .. 10)
        ...
        @test issubset_interval(global_min, 0..1e-20)
        @test length(minimisers) == 1
        @test issubset_interval(minimisers[1], -0.1..0.1)

        global_min, minimisers = minimise(x->(x^2-2)^2, -10..11)
        @test issubset_interval(global_min, 0..1e-7)
        @test length(minimisers) == 2
        @test in_interval(sqrt(2), minimisers[2])
    end
    ...
    for Structure in (SortedVector, HeapedVector)
        ...
        @testset "Minimise in 1D using SortedVector" begin
            ...
            @test issubset_interval(global_min, 0..1e-7)
            @test length(minimisers) == 2
            @test in_interval(sqrt(2), max(minimisers[1], minimisers[2]))
        end
        ...
        @testset "Discontinuous function in 1D" begin
            ...
            @test issubset_interval(global_min, -1 .. -0.9999)
            @test length(minimisers) == 1
            @test in_interval(0, minimisers[1])
        ...
        @testset "Smooth function in 2D" begin
            ...
            @test issubset_interval(global_min, 0..1e-7)
            @test all(all(issubset_interval.(X, [(-1e-3..1e3), (-1e-3..1e-3)])) for X in minimisers)

            # same but with maximise
            global_max, maximisers = maximise( X -> ( (x,y) = X; x^2 + y^2 ), [(-10..10), (-10..10)], structure = Structure )
            @test issubset_interval(global_max, 199.9..200)
            m = (9.99..10)
            @test all(all(issubset_interval.(X, [m, m])) || all(issubset_interval.(X, [-m, m])) || all(issubset_interval.(X, [m, -m])) || all(issubset_interval.(X, [-m, -m])) for X in maximisers)

            # same but with BigFloats
            global_min, minimisers = minimise( X -> ( (x,y) = X; x^2 + y^2 ), [(-big(10.0)..big(10.0)), (-big(10.0)..big(10.0))], structure = Structure )
            @test issubset_interval(global_min, 0..1e-7)
            @test all(all(issubset_interval.(X, [big(-1e-3..1e3), big(-1e-3..1e-3)])) for X in minimisers)
